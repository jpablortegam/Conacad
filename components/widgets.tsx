...
export const StatCard = ({
  ...
  // Obtener las clases de estilo basadas en el tema
  const bgClass = iconBgColor || `icon-bg-${themeColor.toLowerCase()}`;
  const textClass = iconColor || `icon-color-${themeColor.toLowerCase()}`;
  const hoverShadowClass = `hover-shadow-${themeColor.toLowerCase()}`;

  return (
    <Card className={`stat-card group relative ${hoverShadowClass}`}>
      <div className="stat-card-header">
        <div className={`stat-card-icon ${bgClass} ${textClass}`}>
          {icon}
        </div>
...
export const Tag = ({ label, percent, className }: TagProps) => {
  return (
    <div className={`stat-tag ${className}`}>
      {label}: {percent}
    </div>
  );
};
...
export const StatCount = ({
  ...
  // Obtener la clase de gradiente del tema actual
  const gradientClass = className || `gradient-${themeColor.toLowerCase()}`; 
  
  return (
    <div className="stat-count">
      <div className={`stat-count-value ${gradientClass}`}>
        {count}
      </div>
    </div>
  );
...
export const BrowserStats = () => {
  const { themeColor } = useThemeColorContext();
  const iconBg = `icon-bg-${themeColor.toLowerCase()}`;
  const iconColor = `icon-color-${themeColor.toLowerCase()}`;
    ...
    >
      <div className="flex flex-wrap gap-3">
        <Tag
          label="CHROME"
          percent="73%"
          className="tag-chrome"
        />
        <Tag
          label="FIREFOX"
          percent="10%"
          className="tag-firefox"
        />
        <Tag
          label="EDGE"
          percent="11%"
...
export const TeachersStats = () => {
  ...
  const iconBg = `icon-bg-${themeColor.toLowerCase()}`;
  const iconColor = `icon-color-${themeColor.toLowerCase()}`;
  const gradient = `gradient-${themeColor.toLowerCase()}`;
